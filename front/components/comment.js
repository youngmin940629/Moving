...
export default function Comments(props) {
  ...
  const dateFormat = data => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      date +=
        originDate.getMinutes() < 10
          ? '0' + originDate.getMinutes()
  ...
  const submitComment = data => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_BASE_URL}/community/review/${props.data.id}/comment/`,
        {
          user: userID,
      ...
      .then(() => {
        axios
          .get(
            `${process.env.NEXT_PUBLIC_BASE_URL}/community/review/${props.data.id}/`
          )
          .then(res => {
  ...
  const commentDelete = id => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_BASE_URL}/community/comment/${id}/`)
      .then(async () => {
        await axios
          .get(
            `${process.env.NEXT_PUBLIC_BASE_URL}/community/review/${props.data.id}/`
