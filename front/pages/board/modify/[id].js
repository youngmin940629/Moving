...
export default function ModifyBoard({ post, movieTitle }) {
  ...
  const onFinish = data => {
    ...
    data['user'] = userID;
    axios.put(
      `${process.env.NEXT_PUBLIC_BASE_URL}/community/review/${post.id}/`,
      {
        data,
      }
    );
  ...
  const callMovie = async id => {
    await axios
      .get(`${process.env.NEXT_PUBLIC_BASE_URL}/movies/search/${id}`)
      .then(res => {
        setMovies([res.data]);
...
export async function getServerSideProps(context) {
  ...
  if (cookie != null) {
    await axios
      .get(`${process.env.NEXT_PUBLIC_BASE_URL}/community/review/${id}`)
      .then(async res => {
        post = res.data;
        await axios
          .get(`${process.env.NEXT_PUBLIC_BASE_URL}/movies/${post.movie}/`)
          .then(res => {
